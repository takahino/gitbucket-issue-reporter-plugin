...
| `COLUMN_ORDER` | VARCHAR(1000) | 表示する列キーのカンマ区切り順（空文字 = 全17列デフォルト順） |
| `HIGHLIGHT_RULES` | VARCHAR(2000) | ハイライトルールの JSON 配列文字列（空文字 = デフォルトルール） |

`COLUMN_ORDER` に使用できる列キー: `issue_id`, `title`, `status`, `creator`, `assignee`, `milestone`, `labels`, `waiting`, `conf_detail`, `ms_due`, `note`, `comments`, `created_at`, `updated_at`, `start_date`, `end_date`, `progress`
